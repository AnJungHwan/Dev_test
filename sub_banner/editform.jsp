	...
	$.extend({
		confirmDelete : function(global, local, db, page, doc, div) {
			if(!confirm("정말로 삭제하겠습니까?")){
				...
				document.location = "/baas/sub_banner/delete.jsp?global_menu=" + global + "&local_menu=" + local + "&db=" + db + "&page=" + page + "&doc=" +doc + "&div=" +div;
			}
		}
	});
});
//-->
 ...
 <%
 	if (!sessionSuperuserAccessLevel.equals("SUPER")){
   	...
   	SubBannerDataObject dataobj = new SubBannerDataObject();
   	dataobj = (SubBannerDataObject)sqlMap.queryForObject("subBannerArticle",  Integer.toString(docNumber));
...
<table class="normal">	
<form method="post" name="formName" action="/baas/sub_banner/edit.jsp">
...
<tr>
	...
	<select name="division_name" class="thin" style="width:150px;">
		<option value="파인크리크" <% if (dataobj.getDivision_code().equals("PC")) { out.println("selected"); } %>>파인크리크</option>
		<option value="파인밸리" <% if (dataobj.getDivision_code().equals("PV")) { out.println("selected"); } %>>파인밸리</option>
	</select>
	</td>
</tr>	
<tr>
...
<tr>
	...
	<script language="javascript">
			makeSwfSingleUpload(
				...
				file_type_name='그림 및  플래시파일', // 파일선택창 파일형식명 (예: 그림파일, 엑셀파일, 모든파일 등)
