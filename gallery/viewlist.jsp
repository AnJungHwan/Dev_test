...
<tr>
	...
	%>
			    <th style="width: 5%"><%=divisionArray[i][0]%></th>	
	<%
			}
			else {
				
				  //if ( divisionArray[i][1].substring(0,1).equals('C') ) {
			      goURL = strobj.generateForBoardURL(globalMenu, localMenu, dbName, commandName, 1, divisionArray[i][1], divisionArray[i][1]);
			      //goURL = strobj.generateForBoardURL(globalMenu, localMenu, dbName, commandName, 1, 'PC', divisionArray[i][1]);
				  //} else {
				  //    goURL = strobj.generateForBoardURL(globalMenu, localMenu, dbName, commandName, 1, 'PV', divisionArray[i][1]);
				 // }
				  
		%>
		          <td style="width: 5%"><a href="<%=goURL%>"><%=divisionArray[i][0]%></a></td>
	<%
		    }
		}
	%>
...
<tr>
	...
	%>
	<span class="button medium icon"><span class="add"></span><a href="<%=goURL%>">사진 등록</a></span>	
	<%
			}
		%>
	</td>
...
<%
	if (totalCount >0){
		...
		dbList = sqlMap.queryForList("galleryPagingList", pagemap);
		int virtualNumber =0;
		for(int i=0; i < dbList.size(); i++){
			...
			goPic = strobj.generateForGalleryURL(globalMenu, localMenu, dbName, "viewpic", 1, dataobj.getGallery_code(), divisionCode, dataobj.getDivision_code());
