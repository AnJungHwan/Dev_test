...
<tr>
	...
	<%
			...
			goURL = strobj.generateForSearchBoardURL(globalMenu, localMenu, dbName, "searchlist", 1, divisionArray[i][1], divisionArray[i][1], searchWord, searchField);
...
<tr>
	...
	<%
		if(sessionSuperuserAccessLevel.equals("SUPER") || sessionSuperuserDivisionCode.equals(divisionCode)){
			goURL = strobj.generateForBoardURL(globalMenu, localMenu, dbName, "viewlist", pageNumber, divisionCode, divisionCode);	
	%>
...
<%
	if (totalCount >0){
		...
		dbList = sqlMap.queryForList("boardSearchPagingList", pagemap);
		int virtualNumber =0;
...
%>
	<tr><td colspan=8> 검색된  글이  없습니다.</td></tr>
<%
	}
%>
</table><br>
...
<tr>
	...
	if(totalCount >0) {
		...
		if(startPage > 10) {
			...
			goURL = strobj.generateForSearchBoardURL(globalMenu, localMenu, dbName, "searchlist", prevPage, divisionCode, accessCode, searchWord, searchField);
		%>
			<a  href="<%=goURL %>">[이전]</a>
		<%
		}
		for(int i = startPage ; i <= endPage ; i++) {
			if (i==currentPage){
				...
				goURL = strobj.generateForSearchBoardURL(globalMenu, localMenu, dbName, "searchlist", i, divisionCode, accessCode, searchWord, searchField);
		%>	
			<a  href="<%=goURL %>"><%=i %></a>
		<%	
			}
		...
		if (endPage < pageCount) {
			...
			goURL = strobj.generateForSearchBoardURL(globalMenu, localMenu, dbName, "searchlist", nextPage, divisionCode, accessCode, searchWord, searchField);
		%>	
			<a  href="<%=goURL %>">[다음]</a>
		<%	
		}
