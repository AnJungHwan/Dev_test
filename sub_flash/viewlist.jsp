...
<tr>
	...
	%>
	<% if(!divisionCode.equals("AL") ) { %>
	<span class="button medium icon"><span class="refresh"></span><a href="/baas/sub_flash/makexml.jsp?div=<%=divisionCode%>">서브메인(<%=printDivisionName %>)에  반영</a></span>
	<% } %>
	<span class="button medium icon"><span class="add"></span><a href="<%=goURL%>">플래시  추가</a></span>
	</td>
</tr>
</table>
 <table class="list">	
...
<%
	if (totalCount >0){
		...
		skinList = sqlMap.queryForList("subFlashPagingList", pagemap);
		int virtualNumber =0;
		for(int i=0; i < skinList.size(); i++){
			SubFlashDataObject dataobj = (SubFlashDataObject) skinList.get(i);
