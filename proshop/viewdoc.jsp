	...
	$.extend({
		confirmDelete : function(global, local, db, page, div, doc, code) {
			if(!confirm("정말로 삭제하겠습니까?")){
				...
				document.location = "/baas/proshop/delete.jsp?global_menu=" + global + "&local_menu=" + local + "&db=" + db + "&page=" + page + "&div=" + div + "&doc=" +doc +"&code="+code;
			}
		}
	});
});
...
<%
	...
	ProshopDataObject dataobj = new ProshopDataObject();
	dataobj = (ProshopDataObject)sqlMap.queryForObject("proshopArticle", artclemap);
...
<tr>
	<td align="left"><%=dataobj.getUser_id()%>(<%=dataobj.getUser_name()%>)<br><br>    </td>
	<td align="right">
	<%=dataobj.getWrite_date()%> | <%=dataobj.getHitcount()%> hit<br> 
	가격 : <%=dataobj.getPrice()%> 원<br> 
	</td>
</tr>
...
<%
	...
	pnndataobj = (PrevAndNextDataObject)sqlMap.queryForObject("proshopPrevAndNextArticle", pnnmap);
