...
<tr>
	...
	<%
				...
				goURL = strobj.generateForRestaurantURL(globalMenu, localMenu, dbName, commandName, 1, divisionArray[i][1], accessCode);
		%>
...
<tr>
	...
	if(sessionSuperuserAccessLevel.equals("SUPER") || sessionSuperuserDivisionCode.equals(divisionCode)){
		goURL = strobj.generateForRestaurantURL(globalMenu, localMenu, dbName, "writeform", pageNumber, divisionCode, divisionCode, categoryCode);	
	%>
	<span class="button medium icon"><span class="add"></span><a href="<%=goURL%>">메뉴  등록</a></span>
	<%
		}
	%>
	</td>
...
<%
	if (totalCount >0){
		...
		shopList = sqlMap.queryForList("restaurantPagingList", pagemap);
		int virtualNumber =0;
...
%>
	<tr><td colspan=9> 게시된  글이  없습니다.</td></tr>
<%
	}
%>
</table><br>
...
<tr>
	...
	if(totalCount >0) {
		...
		if(startPage > 10) {
			...
			goURL = strobj.generateForRestaurantURL(globalMenu, localMenu, dbName, "viewlist", prevPage, divisionCode, accessCode, categoryCode);
		%>
			<a  href="<%=goURL %>">[이전]</a>
		<%
		}
		for(int i = startPage ; i <= endPage ; i++) {
			if (i==currentPage){
				...
				goURL = strobj.generateForRestaurantURL(globalMenu, localMenu, dbName, "viewlist", i, divisionCode, accessCode, categoryCode);
		%>	
			<a  href="<%=goURL %>"><%=i %></a>
		<%	
			}
		...
		if (endPage < pageCount) {
			...
			goURL = strobj.generateForRestaurantURL(globalMenu, localMenu, dbName, "viewlist", nextPage, divisionCode, accessCode, categoryCode);
		%>	
			<a  href="<%=goURL %>">[다음]</a>
		<%	
		}
