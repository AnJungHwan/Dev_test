...
$(document).ready(function(){
	$.extend({
		confirmDelete : function(global, local, db, page, div, doc, code, category) {
			if(!confirm("정말로 삭제하겠습니까?")){
				return;
			}
			else{
				document.location = "/baas/restaurant/delete.jsp?global_menu=" + global + "&local_menu=" + local + "&db=" + db + "&page=" + page + "&div=" + div + "&doc=" +doc +"&code="+code + "&category=" + category;
			}
		}
	});
});
...
<%
	...
	goList = strobj.generateForRestaurantURL(globalMenu, localMenu, dbName, "viewlist", pageNumber, divisionCode, accessCode, categoryCode);
	goEdit = strobj.generateForRestaurantURL(globalMenu, localMenu, dbName, "editform", pageNumber, docNumber, divisionCode, accessCode, categoryCode);
...
<div class="plate">
	...
	<a href="JavaScript:$.confirmDelete('<%=globalMenu %>','<%=localMenu %>','<%= dbName%>','<%=pageNumber %>','<%=divisionCode %>','<%=docNumber%>','<%=accessCode%>', '<%=categoryCode%>');">삭제</a>
	</span>
	<% } %>
</div>
...
<tr>
	...
	%>
	<img src=/upload/<%=dbName%>/<%= dataobj.getAttach_filename() %>>
	<%
		}
	%><br>
	</td>
...
<%
	...
	pnndataobj = (PrevAndNextDataObject)sqlMap.queryForObject("restaurantPrevAndNextArticle", pnnmap);
...
<tr>
	...
	<% 
		if (nextNumber == 0){
			...
			goURL = strobj.generateForRestaurantURL(globalMenu, localMenu, dbName, "viewdoc", pageNumber, nextNumber, divisionCode, accessCode, categoryCode);
	%>
...
<tr>	
	...
	<% 
		if (prevNumber == 0){
			...
			goURL = strobj.generateForRestaurantURL(globalMenu, localMenu, dbName, "viewdoc", pageNumber, prevNumber, divisionCode, accessCode, categoryCode);
	%>
